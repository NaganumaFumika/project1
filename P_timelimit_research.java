...
public class P_timelimit_research extends JPanel{
    Map map = new Map();
	...
	fr.getContentPane().setBackground(new Color(255, 255, 255));
        P_timelimit_research pr =new P_timelimit_research();
        pr.setOpaque(false);
	fr.add(pr);
	fr.setVisible(true);
    }
...
class NodeList{
   ...
   //pythonにあってjavaにない関数の実装
	...
	for (int i = 0; i < aStar.map_height; i++) {
	    for (int j = 0; j < aStar.map_width; j++) {
		P_timelimit_research.map_result[i][j] = new_map[i][j];
	    }
	}
