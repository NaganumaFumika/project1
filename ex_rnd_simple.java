...
public class ex_rnd_simple extends JPanel{
    Map map = new Map();
	...
	ex_rnd_simple pr =new  ex_rnd_simple();
        pr.setOpaque(false);
	fr.add(pr);
	fr.setVisible(true);
    }
...
class Map extends JPanel{
	...
	try{
	    ...
	    File mapFile = new File("./map5_nopin.png");
	    BufferedImage mapImg = ImageIO.read(new File("./map5_nopin.png"));
...
class NodeList{
    ...
    public void search_path(){
        ...
        for (int i = 0; i < aStar.map_height; i++) {
            for (int j = 0; j < aStar.map_width; j++){
                ex_rnd_simple.map_result[i][j] = newMap[i][j];
