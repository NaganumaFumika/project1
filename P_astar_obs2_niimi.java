...
public class P_astar_obs2_niimi extends JPanel{
    Map map = new Map();
	...
	m.add(map);
	 ...
	 for(int i = 1;i < n ;i++){
		...
		while(true){
		    ...
		    System.out.println(x_rnd+","+y_rnd);
		    if(map.road(x_rnd*xn , y_rnd*yn) == true ){
	...
	addMouseListener(new MouseAdapter() {
		...
		public void mouseClicked(MouseEvent e) {
		    ...
		    if(map.road(e.getX(),e.getY())){
			    ...
			    for (int i = 0;i<n ;i++){
				as.add(new aStar(m.get(i).map)); 
				m.get(i).map = as.get(i).map_result;
			    ...
			    map.map[user.x_now/xn][user.y_now/yn] = 'S';
