...
public class Pstartgoal_bug extends JPanel{
	...
	//バグ用
	Map.button Map_output = map.new button(); //userの周りの50ピクセルの地図を出力
	...
	addMouseListener(new MouseAdapter() {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if(0<=e.getX() && e.getX()<=900 && 0<=e.getY() && e.getY()<=900) {
			    ...
			    } else if(Map_output.x<=e.getX() && e.getX()<=Map_output.x+60 && Map_output.y<=e.getY() && e.getY()<=Map_output.y+60){//バグ用
			    	System.out.println("Map is selected");
			    	int xm = human.get(0).x_now/xn;//マスをだす	    

			    	int ym = human.get(0).y_now/yn;//マスを出す
	...
	fr.getContentPane().setBackground(new Color(255, 255, 255));
        ...
        Pstartgoal_bug pr =new Pstartgoal_bug();
...
class NodeList{
 ...
 //経路探索
	...
	for (int i = 0; i < aStar.map_height; i++) {
		...
		Pstartgoal_bug.map_result[i][j] = new_map[i][j];
