...
public class P_astar_obs_fire extends JPanel{
	...
	fr.getContentPane().setBackground(new Color(255, 255, 255));
        ...
        P_astar_obs_fire pr =new P_astar_obs_fire();
		...
		for(int x = -1*yabakyori;x < yabakyori; x++){
			...
			if (f_x.get(i) + x > -1&& f_x.get(i) + x < lx){
				...
				if(map[f_x.get(i)+x][f_y.get(i)+y]==' '){
				    map[f_x.get(i)+x][f_y.get(i)+y]='P';//PINCHI //Pにhumanが近づきすぎたら再検索するようにする。多分アニメーションのなか
				}
			    }
			}
		    }
