...
public class P_astar_obs extends JPanel{
	...
	addMouseListener(new MouseAdapter() {
		...
		public void mouseClicked(MouseEvent e) {
		    ...
		    if(map.road(e.getX(),e.getY())){
			    ...
			    map.map[user.x_now/30][user.y_now/30] = 'S';
	...
	new javax.swing.Timer(10, new ActionListener(){
		public void actionPerformed(ActionEvent evt){		    
		    int xm = user.x_now/30;
		    int ym = user.y_now/30;
...
class Map {
	...
	for (int x=0; x<xx; x++) {
		...
		if(map[x][y] == 'S' || map[x][y] == 'G' || map[x][y] == ' ' || map[x][y] == '+' ||map[x][y] == 'X' ) {
		    ...
		    if(map[x][y] =='X'){g.drawLine(x*30, y*30, (x+1)*30, (y+1)*30);}
		}
		if(map[x][y] == '0') {
	...
	public void draw (Graphics g){
	    ...
	    if(map[xm][ym]=='+'||map[xm][ym] == 'G'){ g.drawLine(x1,y1,15+30*(xm),15+30*(ym));}
	    int xbf = 2; int ybf = 2;
	    int n = 0;
	    while(true){
		int i = 0;
