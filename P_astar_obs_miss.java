...
public class P_astar_obs2 extends JPanel{
	...
	addMouseListener(new MouseAdapter() {
		int i = 0;
		int s_x, s_y, e_x, e_y;
		int tilex = 900/map.lx;
		int tiley = 900/map.ly;
		public void mouseClicked(MouseEvent e) {
		    System.out.println("click");
		    if(map.road(e.getX(),e.getY())){
		    	if(i == 0){			    
    			    user.movehuman(e.getX() / map.lx * tilex + (tilex/2), e.getY() / map.ly * tiley + (tiley/2));
	    		    map.p.setxy(e.getX() / map.lx * tilex + (tilex/2), e.getY() / map.ly * tiley + (tiley/2));
		    	    map.map[e.getX()/tilex][e.getY()/tiley] = 'S';
			    s_x = e.getX()/tilex;
			    s_y = e.getY()/tiley;
    			    repaint();
	    		    i = 1;
			    ...
			    goal.x = e.getX();
			    goal.y = e.getY();
			    map.map[e.getX()/tilex][e.getY()/tiley] = 'G';
			    map.map[s_x][s_y] = 'S';
			    e_x = e.getX()/tilex;
			    e_y = e.getY()/tiley;
			    for(int i=0; i<map.lx; i++){
			    ...
			    obs.x = e.getX();
			    obs.y = e.getY();
			    map.map[e.getX()/map.lx][e.getY()/map.ly] = 'X';
			    map.map[e_x][e_y] = 'G';
			    repaint();
			    ...
			    map.map[user.x_now/map.lx][user.y_now/map.ly] = 'S';
	...
	new javax.swing.Timer(10, new ActionListener(){
		public void actionPerformed(ActionEvent evt){		    
		    int xm = user.x_now/map.lx;
		    int ym = user.y_now/map.ly;
		    int tilex = 900/map.lx;
		    int tiley = 900/map.ly;

		    if((user.x_now-(tilex/2))%tilex == 0 && (user.y_now-(tiley/2))%tiley == 0){
	    		int i = 0;
		    	map.map[xm][ym] = ' ';
    			for(int x= -1; x < 2 ; x++ ){
...
class Map {
	...
	for (int x=0; x<lx; x++) {
		...
		if(map[x][y] == 'S' || map[x][y] == 'G' || map[x][y] == ' ' || map[x][y] == '+' ||map[x][y] == 'X' ) {
		    ...
		    g.fillRect(x*tilex, y*tiley, tilex , tiley);
		    g.setColor(Color.black);
		    if(map[x][y] =='X'){g.drawLine(x*tilex, y*tiley, (x+1)*tilex, (y+1)*tiley);}
		}
		if(map[x][y] == '0') {
	...
	public void draw (Graphics g){
		...
		g.drawLine(x1,y1,(tilex/2) + lx * (tilex), (tiley/2) + ly * (tiley));
	    }
	    int xbf = 2; int ybf = 2;
	    int n = 0;
	    while(true){
		...
		for(int x= -1; x < 2 ; x++ ){
			...
			for(int y = -1; y < 2; y++){
				...
				if (ym + y > -1 &&  ym + y < ly){
					...
					if(n == 0){
					    if(map[xm][ym]=='+'){ 
						g.drawLine((tilex/2) + xm * tilex, (tiley/2) + ym * tiley , (tilex/2) + tilex * (xm+x), (tiley/2) + tiley * (ym+y));
					    }else{
						g.drawLine(x1,y1,(tilex/2) + tilex * (xm+x), (tiley/2) + tiley * (ym+y));
					    }
					}else{
					    g.drawLine((tilex/2) + xm * tilex, (tiley/2) + ym * tiley, (tilex/2) + tilex * (xm+x) , (tiley/2) +  tiley * (ym+y));
					}
	...
	public void draw(Graphics g) {
	    ...
	    xm = x/lx;
	    ym = y/ly;
	g.fillOval(xm*tilex, ym*tiley, tilex, tiley);
	}
    }
	...
	public void draw(Graphics g) {
	    ...
	    xm = x/lx;
	    ym = y/ly;
	    g.drawLine(xm*tilex, ym*tiley, (xm+1)*tilex, (ym+1)*tiley);
	}
    }
}
