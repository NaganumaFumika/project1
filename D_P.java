//スタート0、ゴール103、keiyuAに経由地、keiyu_xに経由地のx座標、keiyu_yに経由地のy座標
// Dijkstra2 di;のところがうまく行ってない
...
public class D_P extends JPanel{
	...
	for(int i=0; i<di.keiyu.size(); i++) {
	    System.out.println("town: " + di.keiyu.get(i) + " x: " + x.get(i) + " y: " + y.get(i));
	}
...
class Dijkstra2 {
    ...
    ArrayList<Integer> keiyu = new ArrayList<Integer>();
    ArrayList<Integer> keiyu_x = new ArrayList<Integer>(); //経由地のx座標
    ArrayList<Integer> keiyu_y = new ArrayList<Integer>();
	...
	dijkstra(nodeMap, start, goal, distance, via, keiyu, keiyu_x, keiyu_y);	
	...
	if (distance[goal] == Integer.MAX_VALUE) {
	    ...
	    System.out.print(start + " "); //スター
	    for(int i=0; i<keiyu.size(); i++) {
		System.out.print(keiyu.get(i) + " ");
	    }
    ...
    public static void dijkstra(int[][] nodeMap, int start, int goal, int[] distance, int[] via, ArrayList<Integer> keiyu, ArrayList<Integer> keiyu_x, ArrayList<Integer> keiyu_y) {
	...
	//change
	ArrayList<Integer> keiyu_only = new ArrayList<Integer>();
	if (distance[goal] != Integer.MAX_VALUE) {
	...
	for(int i = keiyu_only.size()-1; i>=0; i--) {
	    keiyu.add(keiyu_only.get(i));
	}

	for(int i=0; i<keiyu.size(); i++) {
	    keiyu_x.add( list_x[keiyu.get(i)] );
	    keiyu_y.add( list_y[keiyu.get(i)] );
	    //	    System.out.println("town: " + keiyu.get(i) + " x: " + keiyu_x.get(i) + " y: " + keiyu_y.get(i));
	}
